...
Our team developed a use case centered around a ransomware scenario listed here. This scenario involves CACAO playbook orchestration of OpenC2 commands to the Threat Hunting AP to initiate a Kestrel Hunt on provided STIX data.
...
We are hopeful that in the lead up to the next interworking opportunity we may have the opportunity to improve the demonstration of the existing use case, using actual CACAO instead of pseudo-cacao logic, as well as commands in an official release of the ER AP. Other participants in related areas have opportunity to interface with OIF and the commands it sends, implementing an OC2 Profile function, such as Threat Hunting.
